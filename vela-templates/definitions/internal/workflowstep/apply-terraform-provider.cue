...
template: {
	...
	// The shared credential fields are repeated in each provider below instead of
	// being embedded from a common struct. `parameter` is a disjunction of these
	// structs, and CUE's OpenAPI generator cannot resolve an embedding that sits
	// inside a disjunct: schema generation fails with
	// "unsupported op . for object type" and the definition never syncs (#5767).
	#AlibabaProvider: {
		accessKey!: string
		secretKey!: string
		region!:    string
